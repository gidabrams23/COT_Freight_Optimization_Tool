...
class TrailerAssignmentRuleTests(unittest.TestCase):
    def _optimizer_stub(self):
        ...
        optimizer._plant_optimizer_settings_cache = {}
        return optimizer
    ...
    def test_tractor_supply_long_cargo_item_forces_wedge(self):
        ...
        self.assertEqual(group["wedge_min_item_length_ft"], 20.0)
    ...
    def test_tractor_supply_uta_22ft_item_forces_wedge(self):
        optimizer = self._optimizer_stub()
        optimizer.sku_specs = {
            "SKU-UTA-22": {
                "sku": "SKU-UTA-22",
        ...
        self.assertTrue(group["default_wedge_51"])
        self.assertEqual(group["wedge_min_item_length_ft"], 22.0)
    ...
    def test_auto_hotshot_assignment_respects_plant_level_toggle(self):
        optimizer = self._optimizer_stub()
        optimizer._plant_optimizer_settings_cache = {
            "NV": {"auto_hotshot_enabled": 0},
            "TX": {"auto_hotshot_enabled": 1},
        }
        ...
        updated = optimizer._apply_auto_hotshot_tail_assignments(active_loads, params={})
        self.assertEqual(updated[1]["trailer_type"], "STEP_DECK")
        self.assertEqual(updated[2]["trailer_type"], "HOTSHOT")
    ...
    @patch("services.optimizer.stack_calculator.calculate_stack_configuration")
    def test_step_deck_48_upgrades_to_53_step_before_flatbed(self, mock_calculate):
        optimizer = self._optimizer_stub()
        optimizer._max_stack_for_trailer = lambda _sku, _trailer: 1

        def fake_calculate(_items, trailer_type="STEP_DECK", **_kwargs):
            trailer_key = str(trailer_type or "").strip().upper()
            if trailer_key == "STEP_DECK_48":
                return {"trailer_type": "STEP_DECK_48", "exceeds_capacity": True, "utilization_pct": 90.0}
            if trailer_key == "STEP_DECK":
                return {"trailer_type": "STEP_DECK", "exceeds_capacity": False, "utilization_pct": 80.0}
            if trailer_key == "FLATBED":
                return {"trailer_type": "FLATBED", "exceeds_capacity": False, "utilization_pct": 70.0}
            return {"trailer_type": trailer_key, "exceeds_capacity": False, "utilization_pct": 50.0}
        ...
        config = optimizer._stack_config_for_groups(groups, params, trailer_type="STEP_DECK_48")
        self.assertEqual(config.get("trailer_type"), "STEP_DECK")
        self.assertEqual(config.get("auto_trailer_reason"), "48-foot step deck constraints")
