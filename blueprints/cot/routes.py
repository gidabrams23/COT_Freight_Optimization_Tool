...
logger.info("Blueprint-aware startup: initializing COT and ProGrade blueprints.")
...
cot_bp = Blueprint("cot", __name__)


@cot_bp.before_app_request
...
@cot_bp.app_template_filter("est_datetime")
...
@cot_bp.route("/auth/microsoft/callback")
...
@cot_bp.route("/login", methods=["GET", "POST"])
def login():
...
def _handle_order_upload(file):
    ...
    if discrepancy_entries:
        db.add_upload_load_discrepancies(upload_id, discrepancy_entries)
    discrepancy_rows = db.list_upload_load_discrepancies(upload_id, unresolved_only=True)
    ...
    summary["upload_discrepancies"] = _serialize_upload_discrepancies(
        discrepancy_rows,
        upload_id=upload_id,
    )
    return summary
...
def _serialize_upload_discrepancies(rows, upload_id=None):
    ...
    for row in rows or []:
        ...
        payload = {
            ...
            "tool_load_id": row.get("tool_load_id"),
            "tool_load_number": (row.get("tool_load_number") or "").strip(),
            "tool_load_status": (row.get("tool_load_status") or "").strip().upper(),
    ...
    return {
        ...
        "counts": {
            ...
            "total": len(source_removed) + len(source_unassigned_tool_assigned),
        },
    }
...
def _build_upload_api_response(summary, filename=""):
    ...
    return {
        ...
        "upload_discrepancies": summary.get("upload_discrepancies")
        or _serialize_upload_discrepancies([], upload_id=summary.get("upload_id")),
    }
...
@cot_bp.route("/access/switch", methods=["POST"])
def access_switch():
...
@cot_bp.route("/access/manage", methods=["GET", "POST"])
def access_manage():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/access/delete", methods=["POST"])
def access_delete():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
def _session_load_number_prefix(planning_session, fallback_plant_code=None):
    if not planning_session:
        return ""
    stored_prefix = (planning_session.get("load_number_prefix") or "").strip().upper()
    if stored_prefix:
        return f"{stored_prefix}-"
    plant_code = (
    ...
    if not plant_code:
        return ""
    return f"{plant_code}{_planning_session_year_suffix(planning_session)}-"


def _split_load_number_parts(load_number):
    normalized, suffix = _normalize_load_number(load_number)
    if not normalized:
        return "", ""
    base = normalized
    if suffix == "D":
        base = normalized[:-2] if normalized.endswith("-D") else normalized
    if "-" not in base:
        return "", ""
    prefix_part, sequence_part = base.rsplit("-", 1)
    if len(sequence_part) == 4 and sequence_part.isdigit():
        return f"{prefix_part}-", sequence_part
    return "", ""


def _reserve_session_load_number(
    planning_session,
    fallback_plant_code,
...
@cot_bp.route("/upload", methods=["GET", "POST"])
def upload():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/upload", methods=["POST"])
def orders_upload():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/load-report/upload", methods=["POST"])
...
@cot_bp.route("/api/orders/upload", methods=["POST"])
def api_orders_upload():
    ...
    try:
        ...
        response = _build_blocked_upload_response(exc, filename=getattr(file, "filename", ""))
...
@cot_bp.route("/api/uploads/<int:upload_id>/discrepancies/<int:discrepancy_id>/resolve-remove", methods=["POST"])
def api_resolve_upload_discrepancy_remove(discrepancy_id, upload_id):
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/clear", methods=["POST"])
def clear_orders():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/exclude", methods=["POST"])
def exclude_orders():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/include", methods=["POST"])
def include_orders():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/orders/optimize", methods=["POST"])
def orders_optimize():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/api/orders/manual-validate", methods=["POST"])
...
@cot_bp.route("/api/orders/scope-count", methods=["POST"])
...
@cot_bp.route("/optimize", methods=["GET"])
def optimize():
    return redirect(url_for("orders"))


@cot_bp.route("/optimize/build", methods=["POST"])
def build_optimize():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
def loads():
    ...
    session_load_prefix = ""
    session_next_sequence = None
    if not replay_mode and active_session:
        session_load_prefix = _session_load_number_prefix(active_session)
        try:
            session_next_sequence = (
                int(active_session.get("next_load_sequence"))
                if active_session.get("next_load_sequence") is not None
                else None
            )
        except (TypeError, ValueError):
            session_next_sequence = None
    ...
    for idx, load in enumerate(loads_data, start=1):
        ...
        load["approval_load_number_prefix"] = ""
        load["approval_load_number_suffix"] = ""
        load["approval_load_number_locked"] = False
        if replay_mode:
            continue
        status_value = (load.get("status") or STATUS_PROPOSED).upper()
        if status_value not in {STATUS_PROPOSED, STATUS_DRAFT}:
            continue

        existing_prefix, existing_suffix = _split_load_number_parts(load.get("load_number"))
        if existing_prefix:
            prefix_value = existing_prefix
        elif session_load_prefix:
            prefix_value = session_load_prefix
        else:
            fallback_year_suffix = _planning_session_year_suffix(active_session) if active_session else _year_suffix()
            prefix_value = f"{(load.get('origin_plant') or '').strip().upper()}{fallback_year_suffix}-"
        load["approval_load_number_prefix"] = prefix_value

        if existing_suffix:
            load["approval_load_number_suffix"] = existing_suffix
            load["approval_load_number_locked"] = True
            continue

        if session_next_sequence is not None:
            expected_sequence = session_next_sequence
            load["approval_load_number_suffix"] = f"{expected_sequence:04d}"
...
@cot_bp.route("/planning-sessions")
...
@cot_bp.route("/planning-sessions/replay", methods=["GET", "POST"])
...
@cot_bp.route("/planning-sessions/replay/<int:run_id>")
...
@cot_bp.route("/planning-sessions/replay/<int:run_id>/loads")
...
@cot_bp.route("/planning-sessions/replay/<int:run_id>/reproduce", methods=["POST"])
...
@cot_bp.route("/planning-sessions/replay/<int:run_id>/export.xlsx")
...
@cot_bp.route("/planning-sessions/replay/<int:run_id>/issues.csv")
...
@cot_bp.route("/load-report/<int:session_id>")
...
@cot_bp.route("/load-report/<int:session_id>/export.xlsx")
...
@cot_bp.route("/load-report/<int:session_id>/load/<int:load_id>/sheet.xlsx")
...
@cot_bp.route("/planning-sessions/<int:session_id>")
...
@cot_bp.route("/planning-sessions/<int:session_id>/summary")
...
@cot_bp.route("/planning-sessions/<int:session_id>/archive", methods=["POST"])
...
@cot_bp.route("/planning-sessions/<int:session_id>/delete", methods=["POST"])
def planning_session_delete(session_id):
    abort(404)


@cot_bp.route("/planning-sessions/<int:session_id>/revise")
...
@cot_bp.route("/planning-sessions/<int:session_id>/resume")
...
@cot_bp.route("/loads/manual/suggest")
...
@cot_bp.route("/loads/manual/create", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/manual_add", methods=["POST"])
...
@cot_bp.route("/loads/reopt_jobs/<job_id>")
...
@cot_bp.route("/feedback/app", methods=["POST"])
...
@cot_bp.route("/feedback/app/<int:feedback_id>/resolve", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>")
...
@cot_bp.route("/api/loads/<int:load_id>/route-geometry")
...
@cot_bp.route("/loads/<int:load_id>/carrier", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/trailer", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/reverse-order", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/manifest-sequence", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/schematic")
...
@cot_bp.route("/loads/<int:load_id>/schematic/edit")
...
@cot_bp.route("/loads/<int:load_id>/schematic/edit/save", methods=["POST"])
...
@cot_bp.route("/loads/<int:load_id>/status", methods=["POST"], strict_slashes=False)
def update_load_status(load_id):
    ...
    if action == "approve_lock":
        ...
        custom_suffix_value = (request.form.get("load_number_suffix") or "").strip()
        starting_sequence = None
        ...
        if custom_suffix_value:
            if not LOAD_NUMBER_START_PATTERN.fullmatch(custom_suffix_value):
                return jsonify(
                    {
                        "error": "Load number must be exactly 4 digits.",
                        "invalid_load_number_suffix": True,
                    }
                ), 400
            custom_sequence = int(custom_suffix_value)

        if planning_session and (not load_number or custom_sequence is not None):
            reservation = _reserve_session_load_number(
                planning_session,
                plant_code,
                starting_sequence=starting_sequence,
                requested_sequence=custom_sequence,
            )
            if reservation.get("error") in {"invalid_starting_sequence", "invalid_requested_sequence"}:
                return jsonify({"error": "Load number must be exactly 4 digits."}), 400
        ...
        if not load_number:
            if custom_sequence is not None:
                load_number = _format_load_number(plant_code, year_suffix, custom_sequence, draft=False)
            else:
                seq = db.get_next_load_sequence(plant_code, year_suffix)
                load_number = _format_load_number(plant_code, year_suffix, seq, draft=False)
        else:
            normalized, suffix = _normalize_load_number(load_number)
            base_number = normalized[:-2] if (suffix == "D" and normalized.endswith("-D")) else normalized
            if custom_sequence is not None:
                prefix_value, _ = _split_load_number_parts(base_number)
                if not prefix_value:
                    prefix_value = f"{(plant_code or '').strip().upper()}{year_suffix}-"
                load_number = f"{prefix_value}{custom_sequence:04d}" if prefix_value else base_number
            else:
                load_number = base_number
        ...
        approval_sequence = {}
        if planning_session_id:
            refreshed_session = db.get_planning_session(planning_session_id)
            if refreshed_session:
                next_sequence_value = refreshed_session.get("next_load_sequence")
                try:
                    next_sequence_value = (
                        int(next_sequence_value)
                        if next_sequence_value is not None
                        else None
                    )
                except (TypeError, ValueError):
                    next_sequence_value = None
                approval_sequence = {
                    "prefix": _session_load_number_prefix(refreshed_session, plant_code),
                    "next_sequence": next_sequence_value,
                }
        elif plant_code:
            approval_sequence = {
                "prefix": f"{plant_code}{year_suffix}-",
                "next_sequence": db.get_next_load_sequence(plant_code, year_suffix),
            }
        if is_async:
            snapshot_loads = (
                load_builder.list_loads(
...
@cot_bp.route("/loads/<int:load_id>/remove_order", methods=["GET", "POST"])
...
@cot_bp.route("/loads/<int:load_id>/reject", methods=["POST"])
...
@cot_bp.route("/loads/clear", methods=["POST"])
def clear_loads():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/loads/approve_all", methods=["POST"])
...
@cot_bp.route("/loads/approve_full", methods=["POST"])
...
@cot_bp.route("/loads/reject_all", methods=["POST"])
...
@cot_bp.route("/planning-tools/save", methods=["POST"])
...
@cot_bp.route("/settings/stop-colors/save", methods=["POST"])
...
@cot_bp.route("/rates/fuel-surcharge/save", methods=["POST"])
...
@cot_bp.route("/settings/rates/contexts/save", methods=["POST"])
...
@cot_bp.route("/settings/rates/lst/save", methods=["POST"])
...
@cot_bp.route("/settings/rates/ryder/save", methods=["POST"])
...
@cot_bp.route("/settings/rates/alternate/save", methods=["POST"])
...
@cot_bp.route("/settings/rates/batch-save", methods=["POST"])
...
@cot_bp.route("/settings/global-metrics/save", methods=["POST"])
...
@cot_bp.route("/settings/optimizer-defaults/save", methods=["POST"])
...
@cot_bp.route("/settings/utilization-grades/save", methods=["POST"])
...
@cot_bp.route("/rates/save", methods=["POST"])
def save_rate():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/rates/add", methods=["POST"])
def add_rate():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/rates/delete/<int:rate_id>", methods=["POST"])
...
@cot_bp.route("/skus/save", methods=["POST"])
def save_sku():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/skus/source-led/save", methods=["POST"])
...
@cot_bp.route("/lookups/save", methods=["POST"])
def save_lookup():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/plants/save", methods=["POST"])
def save_plant():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/skus/export-cheat-sheet.xlsx")
...
@cot_bp.route("/skus/add", methods=["POST"])
def add_sku():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/skus/delete/<int:spec_id>", methods=["POST"])
...
@cot_bp.route("/lookups/add", methods=["POST"])
def add_lookup():
    session_redirect = _require_session()
    if session_redirect:
        return session_redirect
...
@cot_bp.route("/lookups/delete/<int:entry_id>", methods=["POST"])
...
@cot_bp.route("/api/orders/<so_num>/stack-config")
...
@cot_bp.route("/api/optimize", methods=["POST"])
...
@cot_bp.route("/api/optimize/<int:run_id>/loads")
...
def _register_legacy_cot_endpoint_aliases(flask_app):
    ...
    for rule in blueprint_rules:
        if rule.endpoint == f"{cot_bp.name}.static":
            continue
        legacy_endpoint = rule.endpoint[len(endpoint_prefix) :]
        if legacy_endpoint in flask_app.view_functions:
            continue
        explicit_methods = sorted(
            method for method in (rule.methods or set()) if method not in {"HEAD", "OPTIONS"}
        )
        flask_app.add_url_rule(
            ...
            methods=explicit_methods or None,
            strict_slashes=rule.strict_slashes,
        )


app.register_blueprint(cot_bp)
app.register_blueprint(prograde_bp)
_register_legacy_cot_endpoint_aliases(app)
